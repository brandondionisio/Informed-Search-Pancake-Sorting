...
def main():
    ...
    UCS.print_result()
    print("Uniform cost search time:", round(timer_stop - timer_start, 2), "seconds")
...
class ucs():
    ...
    def run(self):
        while True:
            ...
            # If the goal_test function returns 0, then we are at the goal
            if curr.goal_test() == 0:
                self.solution = curr
                return
...
class Stack_State:
    ...
    # gets the total cost of the backward cost of the
    # curr node (ucs only relies on backward cost)
    def get_total(self):
    ...
    # goal_test function which is the number of stack positions for
    # which the pancake at that position is not of adjacent size to
    # the pancake below
    # returns the corresponding goal_test integer for the current stack
    def goal_test(self):
        hgap = 0
